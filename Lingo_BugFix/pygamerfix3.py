...
COLOR_TEXT = (0, 0, 0)
COLOR_CORRECT = (0, 255, 0)  # Letter op de juiste positie (groen)
COLOR_WRONG = (255, 165, 0)  # Letter komt voor, maar op andere positie (oranje)
COLOR_NOT_IN_WORD = (128, 128, 128)  # Letter komt niet voor (grijs)
...
GRID_COLS = 5
GRID_ROWS = 6  # Aangepast om het aantal pogingen te overeenkomen
...
def draw_text(text, color, center):
    # Tekst, gecentreerd op de meegegeven positie
    label = FONT.render(text, True, color)
    text_rect = label.get_rect(center=center)
    screen.blit(label, text_rect)


def draw_grid(secret_word, guesses):
    # Individuele cellen
    ...
    for row in range(GRID_ROWS):
        for col in range(GRID_COLS):
            ...
            # Definieer de cel als ne rechthoek met marge
...
def game_loop():
    ...
    while running:
        ...
        # Teken de container voor 't grid
        grid_container_rect = pygame.Rect(SIDE_PADDING - 5, TOP_PADDING + attempts_rect.height + 5,
                ...
                if event.key == pygame.K_BACKSPACE:
                    ...
                    if len(current_guess) == 5:
                        ...
                        if guesses[-1] == secret_word:
                            game_over = True
                            draw_text("Je hebt gewonnen!", COLOR_TEXT, (WIDTH // 2, HEIGHT - 50))
                else:
                    if len(current_guess) < 5 and event.unicode.isalpha():
                        current_guess += event.unicode.lower()

        # Als het spel over is, wacht 2 seconden voordat je teruggaat naar het hoofdmenu
        if game_over and pygame.time.get_ticks() // 1000 >= 2:
            running = False
